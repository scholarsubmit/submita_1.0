...
"""
            mail.send(msg)
            flash(
                f"Verification code sent to {email}. Please check your inbox/spam folder.",
                "success",
            )
...
def create_assignment():
    if request.method == "POST":
        ...
        activity = ActivityLog(
            user_id=current_user.id, action="Created assignment", details=title
        )
        db.session.add(activity)
        db.session.commit()
...
def manage_assignment(assignment_id):
    ...
    avg_grade = (
        sum(s.grade for s in submissions if s.grade) / graded_count
        if graded_count > 0
        else 0
    )
            ...
            if file and file.filename and allowed_file(file.filename):
                if assignment.attachment_path:
                    old_path = os.path.join(
                        app.config["ASSIGNMENT_FOLDER"], assignment.attachment_path
                    )
...
def submit_assignment(assignment_id):
    ...
    if request.method == "POST":
        ...
        if file and file.filename and allowed_file(file.filename):
            ...
            filename = secure_filename(
                f"{current_user.id}_{assignment_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}"
            )
        ...
        activity = ActivityLog(
            ...
            details=f"Assignment: {assignment.title}",
        )
        db.session.add(activity)
        db.session.commit()
        ...
        if plagiarism_detected:
            flash(
                f"⚠️ Warning: Potential plagiarism detected! Similarity: {plagiarism_score:.1f}%",
                "danger",
            )
        else:
            flash(
                f"✓ Assignment submitted successfully! Plagiarism score: {plagiarism_score:.1f}%",
                "success",
            )
        return redirect(url_for("student_dashboard"))
...
def grade_submission(submission_id):
    ...
    return render_template(
        "grade_submission.html", submission=submission, assignment=assignment
    )
...
def export_submissions_csv(assignment_id):
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename={assignment.course_code}_{assignment.title}_submissions.csv"
    )
    return response
...
def export_submissions_excel(assignment_id):
    ...
    return send_file(
        output,
        as_attachment=True,
        download_name=f"{assignment.course_code}_{assignment.title}_submissions.xlsx",
            ...
            if sub1.content and sub2.content:
                ...
                if sim > 0.5:
                    similar_pairs.append(
                        {
                            "student1": sub1.student.name,
                            "student2": sub2.student.name,
                            "similarity": sim * 100,
                        }
                    )
    return render_template(
...
def admin_filter():
    ...
    if filter_type == "all":
        ...
        users = (
            User.query.filter_by(role=role_map[filter_type])
        ...
        submissions = (
            Submission.query.order_by(Submission.submitted_at.desc()).limit(50).all()
        )
        return jsonify(
            {
                "submissions": [
        ...
        assignments = (
            AssignmentSubmission.query.order_by(AssignmentSubmission.created_at.desc())
            .limit(50)
            .all()
        )
        return jsonify(
    ...
    if current_user.is_lecturer():
        submissions = Submission.query.filter(
            Submission.assignment_id.in_(
                db.session.query(AssignmentSubmission.id).filter_by(
                    created_by=current_user.id
                )
            )
        ).all()
