...
You no longer need to run `seed_academic_structure.py` or
`create_admin.py` by hand — `create_app()` (in `app/bootstrap.py`) now
does both automatically on first startup, no matter how the app is
launched (`python run.py`, `flask run`, or gunicorn on Render). It's
idempotent, so it's a no-op on every startup after the first. The
default first-admin login is `scholarsubmit1@gmail.com` / staff ID
`SUBMITA001` / password `1234567890@Me` — **change that password after
your first login.** The two scripts still exist if you ever want to
run them manually (e.g. re-seeding after editing
`academic_structure.json`, or creating an *additional* admin).

**Note on the Postgres driver:** `psycopg[binary]` (the v3 driver) now
lives directly in `requirements.txt`, since Render's default build
command only runs `pip install -r requirements.txt` — a separate
`requirements-postgres.txt` was silently never being installed on
deploy. `psycopg2-binary` also has no prebuilt wheel for newer Python
versions, so v3 avoids that build-tools issue entirely, on Windows and
Linux alike. You don't need to do anything extra for local SQLite
development.
